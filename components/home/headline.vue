<template>
  <div class="mt-5 mx-5 lg:mx-30">
    ...
    <div class="flex flex-col sm:flex-row items-center gap-5 mt-2">
      ...
      />
      <div class="sm:w-6/10">
        <h2 class="text-light transition-all duration-300 ease-linear cursor-pointer" hover="text-bright" @click.prevent="toPost(pinPost.permalink)">{{ pinPost.title }}</h2>
        <p class="text-light/60 mt-2">{{ description }}...</p>
        <div class="flex flex-row gap-2 mt-3">
          <p class="text-light text-sm text-light/60">{{ pinPost.author }},</p>
          <p class="text-light text-sm text-light/60">on {{ pinPost.created_at }}</p>
        </div>
      </div>
    </div>
  </div>
